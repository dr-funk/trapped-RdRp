...
# setting up a default zoom
# motif_tuples contains a sequence and how many residues up/downstream should be included
# motives will be called in order until one is found, starting with aa motives unless --no_amino_acid mode is enabled

# these default ones work well for HAs and RETR/RKKR containign sequences
aamotif_tuples=(('GLFGAIA',-14,8),)
ntsmotif_tuples=(('AAAGAAAAAAAA',-28,38),
                 ('TTTTTTTTCTTT',-38,28),
                 ('AAAGAGAAACAA',-28,38),
                 ('TTGTTTCTCTTT',-28,38))

def find_default_range(seq, motif_type, motif_tuple):
    motif, up, down=motif_tuple

    # first check if the motif is even in the sequence
    if motif not in seq:
        return None
    
    # if it is, the numbers work out slightly different for AAs and nts
    if motif_type=='aa':
        default_range=[(seq.find(motif)+up)*3+frame-0.5, (seq.find(motif)+down)*3+frame-0.5]
    elif motif_type=='nts':
        default_range=[(seq.find(motif)+up)-0.5, (seq.find(motif)+down)-0.5]
...
for record in SeqIO.parse(args.input, 'fasta'):
    ...
    # if we want the pdf or html, we actually have to make the graphs
        ...
        # if we have an AA sequence, scan that first
        if args.no_amino_acid==False:
            for motif_tuple in aamotif_tuples:
                default_range=find_default_range(prot, 'aa', motif_tuple)
                if default_range!=None:
                    break
        
        # if the range has not been found (no AA sequence or not found), scan nts motifs
        if default_range==None:
            for motif_tuple in ntsmotif_tuples:
                default_range=find_default_range(seqs['forward'], 'nts', motif_tuple)
                if default_range!=None:
                    break
        ...
        for fv_nb in range(len(fold_vars)):
            ...
            fig.add_trace(go.Heatmap(z=[results[fv_nb]['forward']['dG']], 
                                    ...
                                    customdata=[[(results[fv_nb]['forward']['struc'][i], 
                                                f"∆G= {results[fv_nb]['forward']['dG'][i]:.2f} kcal/mol" if isinstance(results[fv_nb]['forward']['dG'][i],float) else "no ∆G calculated"
                                                ,i+1,
                                                seqs['forward'][i+1-(local+activelocation):i+1+(footprint-activelocation)+local],
        ...
        fig.add_trace(go.Heatmap(z=[[nts_values[c] for c in seqs['reverse'][::-1]]],
                                x0=1,dx=1,
                                colorscale=nts_colorscale,
                                text=[[c for c in seqs['reverse'][::-1]]],
                                texttemplate='%{text}',
                                customdata=[[[v+1, len(seqs['forward'])-v] for v in range(len(seq))]],
                                hovertemplate='cRNA position: %{customdata[0]}<br>vRNA position: %{customdata[1]}<extra></extra>',
                                showscale=False,
        ...
        for fv_nb in range(len(fold_vars)):
            ...
            fig.add_trace(go.Heatmap(z=[results[fv_nb]['reverse']['dG'][::-1]], 
                                    x0=1,dx=1,
                                    colorscale='purples_r',
                                    customdata=[[(results[fv_nb]['reverse']['struc'][i], 
                                                f"∆G= {results[fv_nb]['reverse']['dG'][i]:.2f} kcal/mol" if isinstance(results[fv_nb]['reverse']['dG'][i],float) else "no ∆G calculated",
                                                i+1,
                                                seqs['reverse'][i+1-(local+activelocation):i+1+(footprint-activelocation)+local],
                                                footprint, activelocation, local
